package AdvanceLectures.excercises
...
class PropertyBasedSet[A](property: A => Boolean) extends MySet[A] {
  ...
  def map[B](f: A => B): MySet[B] = polityFail
  def flatMap[B](f: A => MySet[B]): MySet[B] = polityFail
  def foreach(f: A => Unit): Unit = polityFail
  ...
  def polityFail = throw new IllegalArgumentException("Really deep rabbit hole")
}
